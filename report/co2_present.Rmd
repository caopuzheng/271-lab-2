...
```{r seasonality-adjusted, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r in-sample-fit, echo=FALSE, message=FALSE, warning=FALSE}
...
> Both ARIMA models have similar in-sample fit. The polynomial model has a slightly worse fit.

```{r out-of-sample-forecast, echo=FALSE, message=FALSE, warning=FALSE}
...
polynomial.sa.resid <- fit_cubic_season %>%
  ...
  select(time_index, polynomial_nsa_resid)
...
arima_sa_rmse <- round(sqrt(mean((co2_test$value - arima.sa.forecast$.mean)^2)), 3)
poly_sa_rmse <- round(sqrt(mean((co2_test$value - polynomial.sa.forecast$.mean)^2)), 3)


RMSE <- data.frame(
  model = c("ARIMA Non-Seasonal", "ARIMA Seasonal", "Polynomial Seasonal"),
  rmse = c(arima_nsa_rmse, arima_sa_rmse, poly_sa_rmse)
)
...
> The ARIMA model has a better fit than the polynomial model, and the non-seasonally adjusted ARIMA model has a better fit than the seasonally adjusted ARIMA model.

> Thus we have decided to use the non-seasonally adjusted ARIMA model for our predictions.
...
> CO2 level reached 420 on the 18th week of 2021, and will reach 500 by 2056
