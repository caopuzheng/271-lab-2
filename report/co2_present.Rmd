...
```{r time-plot, echo=FALSE, message=FALSE, warning=FALSE}
co2_present %>%
  ggplot() +
...
```{r, original-linear-model, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r, formal-test-Ljung-Box, echo=FALSE, message=FALSE, warning=FALSE}
...
arima.sa.forecast <- co2.weekly.arima.sa %>%
  ...
  labs(
    x = "Week",
    y = "CO2 Level",
    title = "Out-Of-Sample Seaonally Adjusted ARIMA Model"
  )

arima.nsa.forecast / arima.sa.forecast
```
...
arima_pred <- co2.weekly.arima.nsa %>% forecast(h = 122 * 52)
