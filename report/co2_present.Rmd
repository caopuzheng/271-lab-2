...
p_insample_sa <- in_sample_df %>%
  ggplot(aes(x = time_index)) +
  geom_line(aes(y = value, colour = "Data")) +
  geom_line(aes(y = .fitted_sa+co2_train$seasonal_value, colour = "Fitted")) +
  labs(
    y = "CO2 Level",
    title = "In-Sample 
    Seaonal Adjusted 
...
p_insample_poly <- augment(fit_cubic_season) %>%
  ggplot(aes(x = time_index)) +
  geom_line(aes(y = co2_train$value, colour = "Data")) +
  geom_line(aes(y = .fitted+co2_train$seasonal_value, colour = "Fitted")) +
  labs(
    y = "CO2 Level",
    title = "In-Sample 
...
arima.nsa.forecast <- co2.weekly.arima.nsa %>%
  ...
  mutate(actual_level = co2_test$value)

arima.sa.forecast <- co2.weekly.arima.sa %>%
  forecast(h = nrow(co2_test), level = c(95)) %>%
  mutate(.mean = .mean + co2_test$seasonal_value,
         actual_level = co2_test$value)
...
arima.nsa.forecast.plot <- arima.nsa.forecast %>%
  ggplot(aes(x = time_index)) +
  geom_line(aes(y = actual_level, colour = "Data")) +
...
arima.sa.forecast.plot <- arima.sa.forecast %>%
  ggplot(aes(x = time_index)) +
  geom_line(aes(y = actual_level, colour = "Data")) +
  geom_line(aes(y = .mean, colour = "Fitted")) +
  labs(
...
polynomial.sa.forecast.plot <- polynomial.sa.forecast %>%
  ggplot(aes(x = time_index)) +
  geom_line(aes(y = actual_level, colour = "Data")) +
  geom_line(aes(y = .mean, colour = "Fitted")) +
  labs(
...
(arima.nsa.forecast.plot / arima.sa.forecast.plot) | polynomial.sa.forecast.plot
```
```{r compute-rmse}
