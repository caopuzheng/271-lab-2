...
co2_present <- co2_present %>%
  mutate(
    diff_growth = value - lag(value, n = 52), # Difference in route between years
    annual_growth = diff_growth / lag(value, n = 52) * 100,
    log_value = log(value)
  )
...
```{r ETSDA plots}
co2_present <- co2_present %>%
  mutate(ma_4_week = slider::slide_dbl(value, mean,
...
series <- co2_present %>%
  ggplot() +
  geom_point(aes(x = time_index, y = value)) +
...
# plot acf and pacf
acf <- ggAcf(co2_present$value, ci = 0.95)
pacf <- ggPacf(co2_present$value, ci = 0.95)
