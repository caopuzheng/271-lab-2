...
co2_present_raw <- co2_present_raw %>%
  ...
  mutate(time_index = lubridate::ymd(
    paste(
      year,
      month,
      day,
...
co2_present <- co2_present_raw %>%
  ...
  within(., avg <- ifelse(average.x == -999.99, average.y, average.x)) %>%
  select(time_index, avg)
```
...
co2_present_ts <- co2_present_ts %>%
  mutate(ma_4_week = slider::slide_dbl(avg, mean,
...
series <- co2_present_ts %>%
  ggplot() +
  geom_point(aes(x = time_index, y = avg)) +
  geom_line(aes(x = time_index, y = ma_4_week), color = "red") +
...
# plot acf and pacf
acf <- ggAcf(co2_present_ts$avg, ci = 0.95)
pacf <- ggPacf(co2_present_ts$avg, ci = 0.95)
...
# plot the series
co2_present_ts$diff <- c(0, diff(co2_present_ts$avg))
