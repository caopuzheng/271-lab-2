...
print(end(co2))
```
> The data starts from 1959 Jan and ends in 1997 Dec, with a total 468 monthly observations. There are no missing values.
...
co2_2 <- co2 %>%
  as_tsibble() %>%
  mutate (Diff_growth = value - lag(value), # Difference in route between years
...
p_2 <- co2_2 %>%
  ...
  geom_line(color = 'steelblue') +
  labs(title = TeX(r'(Log Monthly Mean $CO_2$)'),
       subtitle = 'The "Keeling Curve"',
       x = 'Month and Year',
...
p_3 <- co2_2 %>%
  ...
  geom_line(color = 'steelblue') +
  labs(title = TeX(r'(Annualized Growth $CO_2$)'),
       subtitle = 'The "Keeling Curve"',
       x = 'Month and Year',
...
p_5_add <- components(dcmp_add) %>%
  ...
  labs(y = TeX(r'($CO_2$ Annualized Growth Rate)'), 
       x="Month and Year",
...
(p_5_add|p_6_add)/(p_7_add|p_8_add)
...
p_5_multi <- components(dcmp_multi) %>%
  ...
  geom_line(aes(y=trend), colour = "#D55E00") +
  labs(y = TeX(r'(Log $CO_2$ Annualized Growth Rate)'), 
       x="Month and Year",
       title = "Monthly Mean CO2") +
  theme(plot.title = element_text(size=10)) 
