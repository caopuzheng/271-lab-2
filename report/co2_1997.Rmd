...
```
Figure \@ref(fig:CO2-ETSDA) shows that the $CO_2$ trend has steadily increased over time (close to a linear trend line). The annual growth rate is range-bound (mostly 0-0.75%), with a slight upward trend and an average annual long term growth rate of $`r mean_growth_rate`$%.  $CO_2$  is a greenhouse gas, so the increasing trend has significant implications for global warming.  From the histogram chart, we observed that the $CO_2$ levels are not normally distributed, ranging from 310 to 370 ppmv. There are no extreme outlines in this data set. Furthermore, from the  $CO_2$ Decomposition graph, we observed trend, season and irregular components of the data set.  Aside from the trend line discussed above, we observed strong seasonality, and the remaining irregular effect. 
...
```{r model-fitting, echo = FALSE, results = "asis", warning = FALSE}
# use auto
