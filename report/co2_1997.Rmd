...
For the ARIMA model, we can conjecture the following parameters: p: the number of lag observations in the model, also known as the lag order. d: the number of times the raw observations are differenced; also known as the degree of differencing. q: the size of the moving average window, also known as the order of the moving average. Typically for ARIMA, PACF plot dictates the lag order while ACF dictates how many MA terms are required to remove any autocorrelation in the stationary series.

As shown in Figure \@ref(fig:co2-ts-analysis), the time series plot has a strong positive trend and is therefore non-stationary. The ACF plot has significant lags that persist with gradual decay and slight bumps due to seasonality. Applying differencing will remove this trend.
...
adf.test(diff(co2))
```
Figure \@ref(fig:first-order-diff) shows that, after taking a first difference of the data, the time series plot appears to oscillate around 0. To ensure the it is stationary, we applied the Augmented Dickey-Fuller test, which returns a significant p-value less than 0.05. Thus, we have sufficient evidence to reject the null hypothesis and that the time series is stationary. the ACF has cyclic lags that are significant due to seasonality, but there are no signs of dampening. The PACF plot has 3 to 11 leading significant lags until the autocorrelation dampen.
...
# AUTO ARIMA
# TODO: figure out how to do stargazer
...
The final model is estimated to be ARIMA(1,1,1)(1,1,2) with a BIC of 205.5. Checking the residuals of the model in Figure \@ref(fig:arima-model-residuals), the residuals plot oscillates around 0, the ACF autocorrelations are all below or only slightly over the threshold value, and the distribution is Gaussian. The Ljung-Box test returns a large p-value of 0.3406, suggesting that and there are sufficient evidences to reject the null hypothesis and therefore, the residuals are stationary. Since the model residuals are stationary, we decided to perform a forecast on atmospheric CO2 level to 2022 (Figure \@ref(fig:ARIMA-forecast)). Noted that in the figure after 2010, the forecast starts to curve with a wider confidence interval.
...
```{r multiplicative SLT decomposting, echo = FALSE, message = FALSE}
dcmp_multi <- co2_2 %>%
